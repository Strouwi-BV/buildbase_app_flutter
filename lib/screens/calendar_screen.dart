...
class EventTile extends StatelessWidget {
  ...
  const EventTile({Key? key, required this.event, required this.onDelete, required this.onEdit}) : super(key: key);
...
class EventHourTile extends StatelessWidget {
  ...
  final CalendarView calendarView; // Nieuwe property om de kalenderweergave door te geven
  final Function(BuildContext, Event) showEventDetails; // Callback functie voor het tonen van de event details

  const EventHourTile({Key? key, required this.event, required this.index, required this.totalEvents, required this.dayIndex, required this.calendarView, required this.showEventDetails}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final startOfDay = DateTime(event.startTime.year, event.startTime.month, event.startTime.day, 0, 0);
    ...
    if (calendarView == CalendarView.day) {
      ...
      widthPerEvent = (MediaQuery.of(context).size.width - 50) / (totalEvents * 7);
      left = 50 + (index * widthPerEvent) + (dayIndex * (MediaQuery.of(context).size.width - 50) / 7);
    }
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: ListTile(
            title: Text(
              event.title,
              style: TextStyle(fontSize: calendarView == CalendarView.day ? 14 : 10), // Grotere tekst in dagweergave
            ),
            subtitle: calendarView == CalendarView.day
                ? Text(
                    '${event.startTime.hour}:${event.startTime.minute.toString().padLeft(2, '0')} - ${event.endTime.hour}:${event.endTime.minute.toString().padLeft(2, '0')}',
    ...
    calendarBuilders: CalendarBuilders(
      markerBuilder: (context, day, events) {
        if (events.isNotEmpty) {
          return Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: events.map((e) => EventIndicator(event: e as Event)).toList(),
          );
        }
        return null;
      },
                       ...
                       children: [
                         ...
                         ...List.generate(7, (index) {
                           return Positioned(
                             top: 0,
                             bottom: 0,
                             left: 50 + (index * (MediaQuery.of(context).size.width - 50) / 7),
                             child: Container(
                               width: 1,
                           ...
                           child: SizedBox(
                             ...
                             child: ListView.builder(
                               ...
                               itemBuilder: (context, index) {
                                 return HourTile(hour: '${index.toString().padLeft(2, '0')}:00');
                               },
                             ),
                           ),
                         ),
                         ...
                         Positioned(
                          ...
                          child: Text('${day.day}/${day.month}', style: const TextStyle(fontSize: 12)),
                        ),
                      );
                    }),
                  ),
                  ...
                  for (int i = 0; i < 7; i++)
                    for (int j = 0; j < weekEvents[i].length; j++)
                      EventHourTile(
                        ...
                        dayIndex: i,
                        calendarView: _calendarView, // Geef de huidige weergave mee
          ...
          child: SizedBox(
            ...
            child: Stack(
              children: [
                ...
                ...[                            for (int i = 0; i < _getEventsForDay(_selectedDay).length; i++)
                              EventHourTile(
                                event: _getEventsForDay(_selectedDay)[i],
                                index: i,
                                totalEvents: _getEventsForDay(_selectedDay).length,
                                dayIndex: 0,
                                calendarView: _calendarView, // Geef de huidige weergave mee
                                showEventDetails: _showEventDetails
                              ),
